    ...
    for artifact_job in artifact_jobs:
        ...
        if issue_config:
            ...
            while issue_actions:
                ...
                if action.iterate:
                    # we do support only one key
                    keys = list(action.iterate.keys())
                    if len(keys) > 1:
                        raise Exception(
                            "Issue action 'iterate' supports only one key,"
                            f"multiple keys are defined: {keys}")
                    key = keys[0]
                    ctx.logger.debug(f"Processing iteration for key {key}")
                    values = action.iterate[key]
                    if not (isinstance(values, list) and len(values)):
                        raise Exception(
                            f"Iteration key '{key}' should have a list of values defined.")
                    # for each value prepare a separate action
                    for i in range(len(values)):
                        new_action = copy.deepcopy(action)
                        new_action.iterate = None
                        if not new_action.environment:
                            new_action.environment = {key: values[i]}
                        else:
                            new_action.environment[key] = values[i]
                    ...
                    ctx.logger.info(f"Created {len(values)} iterations of action {action.id}")
