...
def get_launch_test_items_data(rp: ReportPortal, launch_id: int, item_type: str) -> dict[str, Any]:
    """Extract test items data from ReportPortal launch.
    ...
    for i in test_items['content']:
        comment = i['issue'].get('comment', '')

        # Collect issue IDs from external system issues
        issue_ids = []
        if i['issue']['externalSystemIssues']:
            for ext in i['issue']['externalSystemIssues']:
                issue_ids.append(ext['ticketId'])
...
def collect_launch_details(
        ...
        rp_url: str,
        rp_project: str,
        launch_id: int) -> tuple[list[str], set[str]]:
    ...
    for item_type in TEST_ITEM_TYPE_MAPPING:
        all_data[item_type] = get_launch_test_items_data(rp, launch_id, item_type)
    ...
    # Print launch header
    output.append(f'Launch name: {launch_details["name"]}')
    output.append(f'URL: {rp_url}/ui/#{rp_project}/launches/all/{launch_id}')
    attrs = ", ".join([f'{a["key"]}={a["value"]}' for a in launch_details["attributes"]])
    output.append(f'Attributes:  {attrs}')
    # Extract first line of description
    description_first_line = launch_details['description'].replace('<br>', '\n').split('\n')[0]
    output.append(f'Description: {textwrap.indent(description_first_line, 2 * " ")}')
    # Print statistics summary
    output.append(format_statistics(launch_details, all_data))
    output.append('')
